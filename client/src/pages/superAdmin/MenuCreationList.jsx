...
import { checkArray, logger } from "../../utils/Helper";
import { getAllMenuServices } from "../../redux/thunk/adminServices";
import store from "../../redux/store";
import MenuCreation from "./MenuCreation";

function MenuCreationList() {
  ...
  const getHandler = async () => {
    try {
      let response = await dispatch(getAllMenuServices()).unwrap();
  ...
  const { allMeanuList } = useSelector((state) => state.adminState);
  console.log("here is my store", allMeanuList);

  console.log(checkArray(allMeanuList), "90909");

  return (
    <div>
      <div className="mb-6 flex items-center justify-between">
        <p className="font-semibold h28">Menu/CrationList</p>
        <div className="flex gap-4">
          <div className="bg-white shadow-xl px-3.5 text-2xl flex justify-center items-center rounded-xl text-gray-400 hover:bg-theme/10 hover:text-theme">
            <IoFilterSharp />
            <span className="text-lg font-light mx-2">Filter</span>
          </div>
      ...
      <div className="grid grid-cols-3 gap-4">
        {checkArray(allMeanuList) &&
          allMeanuList?.map((ele, ind) => {
            return (
              <div className="border p-4 bg-white rounded-xl ">
                <div>
                  <p className="text-black text-sm font-semibold">{`${ele?.menuName}`}</p>
                </div>
                <div>
                  <p className="text-black text-sm font-semibold">{`${ele?.menuDescription}`}</p>
                </div>
                <div>
                  <p className="text-black text-sm font-semibold">{`${ele?.menuOrder}`}</p>
                </div>
              </div>
            );
          })}
      ...
      <MenuCreation isOpen={isOpen} onClose={() => setIsOpen(false)} />
    </div>
  );
}

export default MenuCreationList;
